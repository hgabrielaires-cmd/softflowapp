...
import { Plus, Loader2, Megaphone, Trash2, Search } from "lucide-react";
...
export default function Campanhas() {
  ...
  const { canIncluir: crudIncluir, canEditar: crudEditar } = useCrudPermissions("campanhas", roles);
  ...
  const [novaFilialId, setNovaFilialId] = useState<string>("");
  ...
  async function load() {
    setLoading(true);
    const { data, error } = await supabase.from("crm_campanhas").select("*").order("nome");
  ...
  useEffect(() => { load(); }, []);
  useEffect(() => { if (!novaFilialId && filialPadraoId) setNovaFilialId(filialPadraoId); }, [filialPadraoId]);
  ...
  async function handleAdd() {
    ...
    if (error) toast.error("Erro ao adicionar campanha");
    else { toast.success("Campanha adicionada"); setNova(""); load(); }
    setSaving(false);
  }

  async function toggleAtivo(c: Campanha) {
    await supabase.from("crm_campanhas").update({ ativo: !c.ativo }).eq("id", c.id);
    load();
  }

  async function handleDelete(id: string) {
    const { error } = await supabase.from("crm_campanhas").delete().eq("id", id);
    if (error) toast.error("Erro ao remover campanha. Pode estar vinculada a oportunidades.");
    else { toast.success("Campanha removida"); load(); }
  }

  const nomeFilial = (id: string | null) =>
    filiaisDoUsuario.find(f => f.id === id)?.nome || "—";

  const filtrados = campanhas.filter(c =>
    ...
    <AppLayout>
      <div className="space-y-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Gerencie as campanhas de captação para o CRM</p>
        </div>
        ...
        <div className="bg-card rounded-xl border border-border shadow-card p-4 space-y-3">
          ...
          <div className="flex gap-2 items-end flex-wrap">
            ...
            <div className="min-w-[200px]">
              <Select value={novaFilialId} onValueChange={setNovaFilialId}>
            ...
            <Button onClick={handleAdd} disabled={saving || !nova.trim()} className="gap-2">
