...
export function useContratoGeracaoZapsign({
  ...
  // ── Disparar automação de contrato enviado para assinatura (fire-and-forget) ──
  function dispararAutomacaoContratoEnviado(contratoId: string, signUrl: string, decisorNome: string) {
    supabase.functions.invoke("processar-automacoes", {
      ...
      if (error) console.error("[Automação contrato] Erro:", error);
    }).catch((err) => {
      console.error("[Automação contrato] Erro:", err);
    });
  }
