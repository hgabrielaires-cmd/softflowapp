...
export function OportunidadeComentarios({ oportunidadeId, readOnly = false }: Props) {
  const { user } = useAuth();
  ...
  useEffect(() => { fetchComentarios(); }, [oportunidadeId]);
    ...
    if (likeInfo?.likedByMe) {
      ...
      if (comentario.user_id !== user.id) {
        ...
        await supabase.from("notificacoes").insert({
          ...
          mensagem: `${meuNome} curtiu seu comentário na oportunidade`,
          tipo: "info",
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (file.size > MAX_FILE_SIZE) { toast.error("Arquivo excede o limite de 11 MB."); e.target.value = ""; return; }
    setArquivo(file);
  };
  ...
  const handleEnviar = async () => {
    ...
    try {
      if (arquivo) {
        ...
        const path = `crm/${oportunidadeId}/${crypto.randomUUID()}.${ext}`;
        const { error: uploadErr } = await supabase.storage.from("pedido-anexos").upload(path, arquivo);
  ...
  const handleEnviarResposta = async (parent: Comentario) => {
      ...
      <div key={c.id} className={isReply ? "bg-muted/40 border border-border/50 rounded-md p-2 space-y-0.5" : "bg-background border border-border rounded-md p-2.5 space-y-1"}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <UserAvatar avatarUrl={profile?.avatar_url} fullName={profile?.full_name} size="xs" />
        ...
        {user && (
          <div className="pt-1 flex items-center gap-3">
            <button type="button" onClick={() => handleToggleLike(c)}
              className={`inline-flex items-center gap-1 text-[10px] transition-colors ${likeInfo.likedByMe ? "text-red-500 hover:text-red-600" : "text-muted-foreground hover:text-red-500"}`}>
              <Heart className={`h-3 w-3 ${likeInfo.likedByMe ? "fill-current" : ""}`} />
              {likeInfo.count > 0 && <span>{likeInfo.count}</span>}
            </button>
            {!isReply && (
              <button type="button" onClick={() => { setReplyingTo(replyingTo === c.id ? null : c.id); setReplyTexto(""); }}
      ...
      {!readOnly && (
        <div className="space-y-2 bg-muted/30 rounded-md p-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {PRIORIDADES.map((p) => (
              <button key={p.value} type="button" onClick={() => setPrioridade(p.value)}
                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs border transition-colors ${prioridade === p.value ? "border-primary bg-primary/10 font-semibold" : "border-border hover:border-primary/50"}`}>
                {p.emoji} {p.label}
              </button>
            ))}
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" className="ml-auto text-xs" onClick={handleEnviar} disabled={sending || !texto.trim()}>
              {sending ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1" /> : null} Enviar
            </Button>
          </div>
        </div>
      )}
          ...
          {topLevel.map((c) => {
            ...
            return (
              <div key={c.id} className="space-y-1">
                {renderComment(c)}
                {replies.length > 0 && (
                  <div className="ml-4 space-y-1">{replies.map((r) => renderComment(r, true))}</div>
                )}
                {replyingTo === c.id && (
