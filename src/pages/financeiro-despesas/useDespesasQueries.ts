  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("fin_despesas")
        .select(
          "id, grupo_id, filial_id, fornecedor_id, plano_conta_id, forma_pagamento_id, conta_financeira_id, valor, data_emissao, data_vencimento, descricao, status, parcela_numero, parcela_total, created_at, fornecedores(nome_fantasia), fin_despesa_rateios(centro_custo_id)",
        )
