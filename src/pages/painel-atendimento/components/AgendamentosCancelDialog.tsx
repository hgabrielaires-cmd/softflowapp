...
export function AgendamentosCancelDialog({ open, onOpenChange, agendamentos, onRemover, removendo, onClose }: Props) {
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="max-w-md" onInteractOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          ...
          <DialogDescription>
            Existem {agendamentos.length} compromisso(s) agendado(s) para este projeto cancelado. Deseja removê-los da agenda?
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-2 mt-2">
          <Button variant="destructive" onClick={onRemover} disabled={removendo} className="w-full">
            {removendo ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Trash2 className="h-4 w-4 mr-2" />}
            Remover todos os compromissos
          </Button>
          <Button variant="outline" onClick={onClose} className="w-full">Manter compromissos</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
