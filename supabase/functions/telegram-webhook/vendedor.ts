...
export async function handleVendedorMessage(
  ...
  if (text === "📋 Nova Proposta") {
    await iniciarNovaProposta(supabase, token, chatId, vendedor);
    return;
  }
...
async function iniciarNovaProposta(supabase: any, token: string, chatId: number, vendedor: Vendedor) {
  await supabase
    .from("telegram_pendencias")
  ...
  await supabase.from("telegram_pendencias").insert({
    chat_id: chatId,
    user_id: 0, // preenchido pelo caller externo (telegram user id) — ver index.ts
    etapa: "prop_coleta",
